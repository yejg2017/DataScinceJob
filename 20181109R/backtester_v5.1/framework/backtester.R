...
finalExit <- function(rows, oldPos, sMult) { 
    ...
    slippage <- closeOut(prices, oldPos, sMult)

    # Our final net worth is our portfolio value MINUS amount of slippage
    # incurred to liquidate
    netWorth <- prices$curOp * oldPos - slippage 

    return(list(slippage=slippage, netWorth=netWorth))
}
...
backtest <- function(dataList, getOrders, params, sMult) {
    ...
    for (i in 2: (numOfDays-1)) { # cannot hold on day 1; day 1 data is given to strategy on day 2
        ...
        if(netWorth < 0)
        {
            ...
            pnlRow <- lapply(exit, function(x) x$slippage)
